...
| **라이브 화면 + 소스 스크린** | 콘솔에서 실행 중 브라우저 화면 전환이 실시간으로 흐르고(스텝별 프레임·URL 전환), 그 옆에서 트레이서 이벤트가 SSE로 "지금 도는 소스 라인"을 흘린다 — 화면·소스 동시 관측 |
| **게이트웨이** | 콘솔 `/gallery`가 산출물 인덱스를 직접 서빙(모노레포 프론트 통합 없이도 게이트웨이 성립). `creator export`는 정적 인덱스 |
| **디버거 리플레이** | 캡처된 실행 흐름을 IDE 디버거처럼 line-by-line 스텝 재생 — 현재 라인 하이라이트 + 콜스택 재구성 + **라인별 변수값**(data flow) (`creator debug`) |
| **변수값 추적(data flow)** | `--capture-vars` 시 라인마다 지역변수 값을 스냅샷 — "어느 라인이 돌았나"를 넘어 "그때 변수가 뭐였나"까지 (opt-in, 오버헤드↑) |
| **동시성·멀티스레드** | 트레이스는 논블로킹(진행 중이면 두 번째 요청은 막지 않고 관측 생략·정직 표기), 호출 깊이는 스레드-로컬이라 멀티스레드 핸들러의 스택도 정확 |
